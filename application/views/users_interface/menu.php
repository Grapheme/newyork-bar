	...
	<?php foreach($foods AS $category => $subcategories):?>
		...
		<?php foreach($subcategories AS $subcategory => $food):?>
			...
			<?php foreach($food AS $food_key => $food_value):?>
				...
				<?php if($food_value['id'] == 595 || $food_value['id'] == 596) { ?> <img src="<?=base_url('images/tink.png');?>" alt="" style="position: absolute; left:-50px;"><?php } ?>
				...
				<?php else:?>
					<div class="receipt"><span class="weight"><?=$food_value['weight'];?>&mdash;</span> <br/>&nbsp;</div>
				<?php endif;?>
